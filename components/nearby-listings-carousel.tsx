...
export default function NearbyListingsCarousel({
  ...
  const headingCls = light
    ? "mb-4 sm:mb-6 text-xl font-semibold normal-case tracking-normal text-ink text-left"
    : "mb-4 sm:mb-6 text-lg sm:text-xl md:text-2xl lg:text-3xl tracking-wider text-white uppercase text-left font-gilroy font-medium";
  ...
  if (isLoading) {
    return (
      <section className={`py-8 ${className}`}>
        <h2 className={headingCls}>{t("moreProperties", { city })}</h2>
        <div className="flex gap-3 sm:gap-4 md:gap-5">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className={`flex-none w-64 sm:w-72 md:w-80 h-80 animate-pulse rounded-lg ${
                light ? "bg-neutral-100" : "bg-white/5"
              }`}
            />
          ))}
        </div>
  ...
  return (
    <section className={`py-8 ${className}`}>
      <div className={light ? "mb-2" : "mb-6 sm:mb-8 md:mb-10"}>
        <h2 className={headingCls}>{t("moreProperties", { city })}</h2>
      </div>
        ...
        {maxListings > 0 && (
          <>
            <button
              ...
              className={`absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 sm:-translate-x-2 z-30 w-8 h-8 sm:w-10 sm:h-10 bg-white/90 hover:bg-white disabled:opacity-30 disabled:cursor-not-allowed text-ink hover:text-ink rounded-full shadow-lg border transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-ink/40 focus:ring-offset-2 ${
                light ? "border-neutral-200" : "border-white/20"
              }`}
          ...
          {listings.map((listing) => {
            ...
            return (
              <Link
                ...
                className={`flex-none w-64 sm:w-72 md:w-80 lg:w-72 xl:w-80 2xl:w-96 group cursor-pointer rounded-lg overflow-hidden relative border focus:outline-none focus:ring-2 focus:ring-ink/20 transition-all duration-300 hover:scale-[1.02] ${
                  light
                    ? "border-neutral-200 hover:border-neutral-300"
                    : "border-white/10 hover:border-[#ffffff]/40"
                }`}
              >
                <div className="aspect-[3/4] relative">
                  {listing.listingImages?.[0] || listing.images?.[0] ? (
                    <Image
                      ...
                      alt={displayPropertyName(listing)}
                      fill
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 text-white">
                    <h3 className="text-sm sm:text-base font-poppins font-semibold text-left mb-1">
                      {displayPropertyName(listing)}
                    </h3>
                    <p className="text-xs text-white/70 font-poppins text-left mb-1">
        ...
        <div className="flex justify-center space-x-2 mb-6 sm:mb-8">
          {Array.from({ length: maxListings + 1 }, (_, index) => (
            <button
              ...
              className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-ink/40 focus:ring-offset-2 ${
                light ? "" : "focus:ring-offset-black"
              } ${
                currentListing === index
                  ? `w-6 sm:w-7 ${light ? "bg-ink" : "bg-[#ffffff]"}`
                  : light
